...
async function openDetails(id, type){
  ...
  const poster = d.poster_path ? (imgBase + d.poster_path) : '';
  const title  = (d.title || d.name || '').trim();
  link.href = `/player.html?src=${encodeURIComponent(w)}&title=${encodeURIComponent(title)}&poster=${encodeURIComponent(poster)}`;
